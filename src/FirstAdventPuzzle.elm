module FirstAdventPuzzle exposing (..)

import Task


solvePart1 : (Int -> msg) -> Cmd msg
solvePart1 toMsg =
    puzzle
        |> String.split "\n"
        |> List.map extractFirstAndLastDigit
        |> List.sum
        |> (\code -> Task.perform toMsg (Task.succeed code))


extractFirstAndLastDigit : String -> Int
extractFirstAndLastDigit lineOfText =
    String.filter Char.isDigit lineOfText
        |> (\filteredLineOfText -> String.left 1 filteredLineOfText ++ String.right 1 filteredLineOfText)
        |> String.toInt
        |> Maybe.withDefault 0
...
solvePart2 toMsg =
    puzzle
        |> String.lines
        |> List.map (\lineOfText -> ( firstNumberFromString lineOfText "", secondNumberFromString lineOfText "" ))
        |> List.map
            (\tuple ->
                case tuple of
                    ( Just number1, Just number2 ) ->
                        Just ( number1, number2 )
        ...
        |> List.map
            (\( number1, number2 ) ->
                String.concat [ String.fromInt number1, String.fromInt number2 ]
                    |> String.toInt
            )
        |> List.filterMap identity
        |> List.sum
...
firstNumberFromString : String -> String -> Maybe Int
firstNumberFromString uncheckedString checkedString =
    if not (String.isEmpty uncheckedString) then
        case ( String.toInt (String.left 1 uncheckedString), textNumberToInt (checkedString ++ String.left 1 uncheckedString) ) of
            ( Just number, Nothing ) ->
                Just number
            ...
            ( Nothing, Nothing ) ->
                firstNumberFromString
                    (String.dropLeft 1 uncheckedString)
                    (checkedString ++ String.left 1 uncheckedString)

    else
        Nothing


secondNumberFromString : String -> String -> Maybe Int
secondNumberFromString uncheckedString checkedString =
    if not (String.isEmpty uncheckedString) then
        case ( String.toInt (String.right 1 uncheckedString), textNumberToInt (String.right 1 uncheckedString ++ checkedString) ) of
            ( Just number, Nothing ) ->
                Just number
            ...
            ( Nothing, Nothing ) ->
                secondNumberFromString
                    (String.dropRight 1 uncheckedString)
                    (String.right 1 uncheckedString ++ checkedString)

    else
        Nothing


textNumberToInt : String -> Maybe Int
textNumberToInt text =
    if String.contains "one" text then
        Just 1

    else if String.contains "two" text then
        Just 2

    else if String.contains "three" text then
        Just 3

    else if String.contains "four" text then
        Just 4

    else if String.contains "five" text then
        Just 5

    else if String.contains "six" text then
        Just 6

    else if String.contains "seven" text then
        Just 7

    else if String.contains "eight" text then
        Just 8

    else if String.contains "nine" text then
        Just 9
