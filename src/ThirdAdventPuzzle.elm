module ThirdAdventPuzzle exposing (..)
...
onlyNumbersAroundSymbol : Int -> List { number : Int, indexes : List Int } -> List { number : Int, indexes : List Int }
onlyNumbersAroundSymbol symbolIndex lineWithWholeNumbersAndIndexes =
    lineWithWholeNumbersAndIndexes
        |> List.filter
            (\{ indexes } ->
                List.member symbolIndex indexes
                    || List.member (symbolIndex - 1) indexes
                    || List.member (symbolIndex + 1) indexes
            )


linesWithWroleNumbersAndIndexes : Dict.Dict Int (List { indexes : List Int, number : Int })
linesWithWroleNumbersAndIndexes =
    puzzle
        |> String.lines
        |> List.map String.trim
        |> List.map
            (\line ->
                String.toList line
                    |> List.indexedMap Tuple.pair
                    |> List.filter (Tuple.second >> Char.isDigit)
                    |> (\digitsWithIndexes ->
                            toWholeNumbersWithIndexes digitsWithIndexes []
                       )
            )
        |> List.indexedMap Tuple.pair
        |> Dict.fromList
...
toWholeNumbersWithIndexes : List ( Int, Char ) -> List { indexes : List Int, number : Int } -> List { indexes : List Int, number : Int }
toWholeNumbersWithIndexes digitsWithIndexes listed =
    digitsWithIndexes
        |> List.head
        |> Maybe.map
            (\( index, char ) ->
                let
                    mapListed =
                        listed
                            |> List.map
                                (\{ indexes, number } ->
                                    if List.any (\a -> (a == index) || (a == (index - 1)) || (a == (index + 1))) indexes then
                                        { indexes = index :: indexes
                                        , number = String.fromInt number ++ String.fromChar char |> String.toInt |> Maybe.withDefault 0
                                        }

                                    else
                                        { indexes = indexes, number = number }
                                )
                in
                if mapListed == listed then
                    toWholeNumbersWithIndexes
                        (List.drop 1 digitsWithIndexes)
                        ({ indexes = [ index ], number = char |> String.fromChar |> String.toInt |> Maybe.withDefault 0 } :: listed)
