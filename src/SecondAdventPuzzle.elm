module SecondAdventPuzzle exposing (..)
...
solvePart1 toMsg =
    puzzle
        ...
        |> List.sum
        |> (\secondPuzzleResult -> Task.perform toMsg (Task.succeed secondPuzzleResult))
...
solvePart2 toMsg =
    puzzle
                                        ...
                                        biggestBlue =
                                            throws
                                                |> List.map
                                                    (\cubeDrop ->
                                                        String.split "," cubeDrop
                                                            |> List.map (getValueOf Blue)
                                                            |> List.sum
                                                    )
                                                |> List.maximum

                                        power =
                                            [ biggestRed, biggestGreen, biggestBlue ]
                                                |> List.filterMap identity
                                                |> List.product
                                    in
                                    power
        ...
        |> List.sum
        |> (\secondPuzzlePart2Result -> Task.perform toMsg (Task.succeed secondPuzzlePart2Result))
...
getValueOf : Color -> String -> Int
getValueOf requiredColor string =
    checkColor string
        |> Maybe.andThen
            (\color ->
...
checkValidCubeDrop : String -> Maybe Bool
checkValidCubeDrop string =
    checkColor string
        |> Maybe.andThen
            (\color ->
                String.filter Char.isDigit string
...
checkColor : String -> Maybe Color
checkColor string =
    if String.contains "red" string then
        Just Red

    else if String.contains "green" string then
        Just Green

    else if String.contains "blue" string then
        Just Blue
